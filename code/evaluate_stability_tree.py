...
def fit_predict_yadt_decision_tree_param(train_test, seed, features):
    ...
    clf = yadt.YaDTClassifier(metadata, options='-m %s' % yadt_m)
...
def set_argparser():
    ...
    parser.add_argument('-y', dest='y', help='Yadt m', default=2, type=int)
    return parser
...
def run_evaluate_stability(dataset_name, model_name, datasets_path='', models_path='', results_path='',
    ...
    preprocessing_pipe = prep.build_preprocessing_pipe() #[0:1]
    ...
    if not load_model:
        ...
        stability.store_model(trained_model, (model_name[0], model_name[1] + '_m%d' % yadt_m), dataset_name, models_path)
    else:
        if verbose:
    ...
    stability.store_model_stability(model_stability, model_stability_comparison, aggregation_functions,
                                    (model_name[0], model_name[1] + '_m%d' % yadt_m), dataset_name, results_path)

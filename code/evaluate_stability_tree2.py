...
def fit_predict_yadt_decision_tree_param(train_test, seed, features):
    global yadt_depth
    X_train, X_test, y_train, y_test, fsindexes = train_test
    metadata = [f for fs, f in zip(fsindexes, features) if fs]
    if yadt_depth is not None:
        clf = yadt.YaDTClassifier(metadata, options='-depth %s' % yadt_depth)
    else:
        clf = yadt.YaDTClassifier(metadata)
...
fit_predict_model = {
    ...
    ('DT', 'yadt'): fit_predict_yadt_decision_tree_param,
...
def set_argparser():
    ...
    parser.add_argument('-depth', dest='depth', help='Yadt depth', default=None, type=int)
    return parser
...
def run_evaluate_stability(dataset_name, model_name, datasets_path='', models_path='', results_path='',
    ...
    preprocessing_pipe = prep.build_preprocessing_pipe_yadt2()
    ...
    if not load_model:
        ...
        stability.store_model(trained_model, (model_name[0], model_name[1] + '_depth%s' % yadt_depth), dataset_name, models_path)
    else:
        if verbose:
    ...
    stability.store_model_stability(model_stability, model_stability_comparison, aggregation_functions,
                                    (model_name[0], model_name[1] + '_depth%s' % yadt_depth), dataset_name, results_path)
